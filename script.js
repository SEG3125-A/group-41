...
function populateProducts() {
  ...
  for (i = 0; i < checkedCategory.length; i++){
    productsToAdd.push.apply(productsToAdd, getProducts(checkedCategory[i].value, checkedRestriction));
  }
  if (checkedCategory.length == 0) {
    for (i = 0; i < productCategory.length; i++) {
      productsToAdd.push.apply(productsToAdd, getProducts(null, checkedRestriction));
    }
  }
